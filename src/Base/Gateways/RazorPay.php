...
class RazorPay extends MultiGatewayBase
{
    /*
    * charge_amount();
    * @required param list
    ...
    * */
    ...
    public function charge_customer($args)
    {
        ...
        abort_if(is_null(config('multigateway.razorpay.api_key')),405,'razorpay api key is missing');

        return view('multigateway::razorpay')->with('razorpay_data', $razorpay_data);
    }
    ...
    public function ipn_response($args = []){
        ...
        abort_if(is_null(config('multigateway.razorpay.api_key')),405,'razorpay api key is missing');
        abort_if(is_null(config('multigateway.razorpay.api_secret')),405,'razorpay api secret is missing');
        //get API Configuration
        $api = new Api(config('multigateway.razorpay.api_key'), config('multigateway.razorpay.api_secret'));
