...
namespace Turtlecap\Multigateway\Base\Gateways;

use Turtlecap\Multigateway\Base\MultiGatewayBase;
use Srmklive\PayPal\Services\PayPal as PayPalClient;

class PaypalPay extends MultiGatewayBase
{
    /*
* charge_amount();
* @required param list
...
* */
    ...
    protected function getPaymentProvider($args){
        ...
        $config = [
            'mode'    => config('multigateway.paypal.mode'),
            'sandbox' => [
                'client_id'         => config('multigateway.paypal.sandbox.client_id'),
                'client_secret'     => config('multigateway.paypal.sandbox.client_secret'),
                'app_id'            => config('multigateway.paypal.sandbox.app_id'),
            ],
            'live' => [
                'client_id'         => config('multigateway.paypal.live.client_id'),
                'client_secret'     => config('multigateway.paypal.live.client_secret'),
                'app_id'            => config('multigateway.paypal.live.app_id'),
            ],

            'payment_action' => 'Sale',
            'currency'       => config('multigateway.global_currency'),
       ...
       $provider = $this->getPaymentProvider($args);
        ...
        $order = $provider->createOrder([
            ...
            "purchase_units"=> [
                0 => [
                    "amount"=> [
                        "currency_code"=> config('multigateway.global_currency'),
                        "value"=> $args['amount']
                    ]
                ]
            ],
