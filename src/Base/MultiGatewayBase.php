...
abstract class MultiGatewayBase
{
    protected $config;
    ...
    protected static function global_currency(){
        return config('multigateway.global_currency');
    }
    ...
    protected static function make_amount_in_usd($amount,$currency){
        ...
        foreach ($all_currency as $cur => $symbol) {
            ...
            if ($cur === $currency) {
                $exchange_rate = config('multigateway.usd_exchange_rate'); // exchange rate
   ...
   private static function make_amount_in_inr($amount, $currency)
    {
        ...
        foreach ($all_currency as $cur => $symbol) {
            ...
            if ($cur == $currency) {
                $exchange_rate = config('multigateway.inr_exchange_rate');
                $output = $amount * $exchange_rate;
            }
        }
    ...
    private static function make_amount_in_idr($amount, $currency)
    {
        ...
        foreach ($all_currency as $cur => $symbol) {
            ...
            if ($cur == $currency) {
                $exchange_rate = config('multigateway.idr_exchange_rate');
                $output = $amount * $exchange_rate;
            }
        }
    ...
    private static function make_amount_in_ngn($amount, $currency)
    {
        ...
        foreach ($all_currency as $cur => $symbol) {
            ...
            if ($cur === $currency) {
                $exchange_rate = config('multigateway.ngn_exchange_rate');
                $output = $amount * $exchange_rate;
            }
        }
    ...
    protected static function make_amount_in_zar($amount,$currency){
        ...
        foreach ($all_currency as $cur => $symbol) {
            ...
            if ($cur == $currency) {
                $exchange_rate = config('multigateway.zar_exchange_rate');
                $output = $amount * $exchange_rate ;
            }
        }
    ...
    protected static function make_amount_in_brl($amount,$currency){
        ...
        foreach ($all_currency as $cur => $symbol) {
            ...
            if ($cur == $currency) {
                $exchange_rate = config('multigateway.brl_exchange_rate');
                $output = $amount * $exchange_rate ;
            }
        }
